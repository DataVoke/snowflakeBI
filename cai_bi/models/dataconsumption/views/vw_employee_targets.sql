...
round(billable_hours_per_week*week_num,1) as ytd_target, round(ytd_actual/decode(ytd_target,0,1,ytd_target) * 100,1) as ytd_percentage from {{ref('dim_timesheet_entry_aggregates')}} where 
...
round(sum(expected_hours),1) as l12m_target, round(l12m_actual/decode(l12m_target,0,1,l12m_target) * 100,1) as l12m_percentage from {{ref('dim_timesheet_entry_aggregates')}} where 
