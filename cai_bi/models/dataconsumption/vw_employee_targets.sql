...
with 
ytd as (
select ukg_employee_number, round(expected_hours,1) as billable_hours_per_week, round(52*expected_hours,1) as billable_hours_per_year, count(date_group_id) as week_num , round(sum(hours)/week_num, 1) as ytd_average_per_week, round(sum(hours),1) as ytd_actual,
...
last_12_mths as (
select ukg_employee_number, count(date_group_id) as week_num ,   round(sum(expected_hours)/week_num,1) as billable_hours_per_week, round(52*billable_hours_per_week,1) as billable_hours_per_year,  round(sum(hours)/week_num, 1) as l12m_average_per_week, round(sum(hours),1) as l12m_actual,
...
ytd_actual / decode(ytd.billable_hours_per_year,0,1,ytd.billable_hours_per_year ) *100 as progress
